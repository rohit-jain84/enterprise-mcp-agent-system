    ...
    <div className="h-full overflow-y-auto">
      <div className="max-w-4xl mx-auto p-6">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <div className="card">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">{servers.length}</div>
                <div className="text-xs text-gray-500 dark:text-slate-400">Total Servers</div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-emerald-50 dark:bg-emerald-500/20 rounded-lg flex items-center justify-center">
                <Activity size={20} className="text-emerald-500 dark:text-emerald-400" />
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">{connectedCount}</div>
                <div className="text-xs text-gray-500 dark:text-slate-400">Connected</div>
              </div>
            </div>
          </div>
          ...
          <div className="card">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 bg-purple-50 dark:bg-purple-500/20 rounded-lg flex items-center justify-center">
                <Wrench size={20} className="text-purple-500 dark:text-purple-400" />
              </div>
              <div>
                <div className="text-2xl font-bold text-gray-900 dark:text-slate-100">{totalTools}</div>
                <div className="text-xs text-gray-500 dark:text-slate-400">Available Tools</div>
              </div>
            </div>
          </div>
        </div>

        <h2 className="text-lg font-semibold text-gray-800 dark:text-slate-200 mb-4">MCP Servers</h2>
        ...
        {loading ? (
          ...
          <EmptyState
            icon={<Server size={48} className="text-gray-400 dark:text-slate-500" />}
          ...
          <div className="space-y-3">
            {servers.map((server) => (
              <div key={server.id} className="card">
                <div className="flex items-start justify-between mb-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-gray-400 dark:text-slate-500 font-mono">{server.url}</p>
                    </div>
                  </div>
                ...
                {server.error && (
                  <div className="mt-2 text-xs text-red-600 dark:text-red-400 bg-red-50 dark:bg-red-500/10 rounded-lg p-2">
                    {server.error}
                  </div>
                )}
              </div>
